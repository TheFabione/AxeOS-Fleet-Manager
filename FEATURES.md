...
- **Export and import devices**: back up the device list with all metadata (nicknames, labels, custom icons, custom order) as JSON and restore on a fresh install
...
### Custom Device Icons
- Pick one of 10 icons per device (axe, chip, CPU, server, rocket, flame, bolt, diamond, pickaxe, star), or reset to the type default. Per-device, persisted.
### Card View Modes
...
- **Flap Recovery**: auto-restart a device when it cycles offline/online above a configurable threshold within a time window; with built-in cooldown to prevent loops
### Themes & Accessibility
- **Two dark themes**: Neon Vault (lime/violet, default) and Aurora (cyan/magenta), with typography Syne (display), Sora (body), Space Mono (data)
...
- **themes.css**: 2 dark theme definitions using CSS custom properties
